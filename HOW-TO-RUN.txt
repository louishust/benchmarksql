...
5. Build the schema and initial database load(for cockroach)

    Cockroachdb is not compatible with PG completely, So we should init data in pg first like step 4, then import data from pu_dump.

    [wieck@localhost run]$ psql -p 26257 -h localhost -d benchmarksql -U root< tableCreates.sql
    CREATE TABLE
    CREATE TABLE
    CREATE TABLE
    CREATE TABLE
    ...
    [wieck@localhost run]$  pg_dump -p5432 -h localhost -d benchmarksql -U root > 1warehouses-cock.sql

    Modify the 1warehouses-cock.sql according [cockroach import data](https://www.cockroachlabs.com/docs/stable/import-data.html), make sure the sql file just contain `COPY` statement.

    [wieck@localhost run]$  psql -p26257 -h localhost -d benchmarksql -U root < 1warehouses-cock.sql

    [wieck@localhost run]$  psql -p26257 -h localhost -d benchmarksql -U root < foreignKeys.sql


6. Run the configured benchmark
...
7. Scale the benchmark configuration.
